...
void messageReceived(String &topic, String &payload) {
  ...
  if (topic == "/home/mafos/config/ota") {
...
void connectToMQTT() {
  ...
  Serial.print("\nConnecting to mqtt...");
  while (!client.connect("mafos", MQTT_USERNAME, MQTT_PASSWORD)) {
    Serial.print(".");
    delay(1000);
  }
  Serial.println("");
...
void subscribeToConfigChannel() {
  client.subscribe("/home/mafos/config/ota");
}

void publishSensorsData(SensorData &data) {
  client.publish("/home/mafos/environment/temperature", String(data.temperature));
  client.publish("/home/mafos/environment/humidity", String(data.humidity));    
  client.publish("/home/mafos/environment/light-intensity", String(data.lightIntensity));
}
